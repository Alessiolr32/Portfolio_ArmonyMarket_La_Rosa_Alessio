<x-layouts.layout>
    <x-slot:title>{{__('ui.articles_list')}}</x-slot:title>
    
    <header class="text-center my-5 rounded-5 shadow w-75 mx-auto header-section">        
        ...
        @else
        <h1 class="h2 fw-bold mb-0 mt-0">{{__('ui.all_the_articles')}}</h1>                   
        <p class="text-black mb-0">{{__('ui.complete_list')}}</p>
        @endif                
    </header>
    ...
    <div class="container my-5">           
        <section class="row g-4 card-bg-color p-3 pt-1 pb-4 shadow mt-4">
            ...
            @endif
            @empty
            <p class="text-center text-muted fs-6 m-0 mt-3 p-1">
                {{__('ui.no_items_available')}}
            </p>
            @endforelse
        </section>
